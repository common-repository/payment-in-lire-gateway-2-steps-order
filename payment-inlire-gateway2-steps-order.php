<?php
/**
 * Plugin Name: Payment in-Lire Gateway 2 Steps Order
 ...
 * WC tested up to: 3.6
 * Text Domain: payment-inlire-gateway2-steps-order
...
function woocommerce_inlire_wc_missing() {
	...
	echo '<div class="error"><p><strong>' . sprintf( esc_html__( 'in-Lire requires WooCommerce to be installed and active. You can download %s here.', 'payment-inlire-gateway2-steps-order' ), '<a href="https://woocommerce.com/" target="_blank">WooCommerce</a>' ) . '</strong></p></div>';
}
	...
	if ( ! class_exists( 'WC_InLire' ) ) :
		...
		class WC_InLire {
			...
			public function plugin_action_links( $links ) {
				$plugin_links = array(
					'<a href="admin.php?page=wc-settings&tab=checkout&section=inlire">' . esc_html__( 'Settings', 'payment-inlire-gateway2-steps-order' ) . '</a>'
				);
