		...
		if ( isset( $options['enabled'] ) && 'yes' === $options['enabled'] ) {
			if ( empty( $show_phpver_notice ) ) {
				if ( version_compare( phpversion(), WC_INLIRE_MIN_PHP_VER, '<' ) ) {
					...
					$message = __( 'in-Lire - The minimum PHP version required for this plugin is %1$s. You are running %2$s.', 'payment-inlire-gateway2-steps-order' );
			...
			if ( empty( $show_wcver_notice ) ) {
				if ( version_compare( WC_VERSION, WC_INLIRE_MIN_WC_VER, '<' ) ) {
					/* translators: 1) int version 2) int version */
					$message = __( 'in-Lire - The minimum WooCommerce version required for this plugin is %1$s. You are running %2$s.', 'payment-inlire-gateway2-steps-order' );
			...
			if ( empty( $show_curl_notice ) ) {
				if ( ! function_exists( 'curl_init' ) ) {
					$this->add_admin_notice( 'curl', 'notice notice-warning', __( 'in-Lire - cURL is not installed.', 'payment-inlire-gateway2-steps-order' ), true );
				}
			}
			
			if ( empty( $show_urlservice_notice ) ) {
				if ( empty( $options['payment_service_url'] ) ) {
					$this->add_admin_notice( 'urlservice', 'notice notice-warning', __( 'in-Lire - Insert the url', 'payment-inlire-gateway2-steps-order' ));
				}
			}
			
			if ( empty( $show_token_notice ) ) {
				if ( empty( $options['payment_token_key'] ) ) {
					$this->add_admin_notice( 'token', 'notice notice-warning', __( 'in-Lire - Insert the token', 'payment-inlire-gateway2-steps-order' ));
				}
			}
			
			if ( empty( $show_maxamount_notice ) ) {
				if ( empty( $options['order_amout'] ) && !is_numeric($options['order_amout']) ) {
					$this->add_admin_notice( 'maxamount', 'notice notice-warning', __( 'in-Lire - Insert the maximum amount for the orders', 'payment-inlire-gateway2-steps-order' ));
				} else {
					if (($options['order_amout'] <= 0))
					$this->add_admin_notice( 'maxamount', 'notice notice-warning', __( 'in-Lire - Order amount must be greater then 0', 'payment-inlire-gateway2-steps-order' ));
				}
			}
		}
	}
	...
	public function hide_notices() {
		if ( isset( $_GET['wc-inlire-hide-notice'] ) && isset( $_GET['_wc_inlire_notice_nonce'] ) ) {
			if ( ! wp_verify_nonce( $_GET['_wc_inlire_notice_nonce'], 'wc_inlire_hide_notices_nonce' ) ) {
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'payment-inlire-gateway2-steps-order' ) );
			}

			if ( ! current_user_can( 'manage_woocommerce' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'payment-inlire-gateway2-steps-order' ) );
			}
