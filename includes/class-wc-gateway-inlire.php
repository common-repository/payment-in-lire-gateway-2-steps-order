...
class WC_Gateway_InLire extends WC_InLire_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->id             = 'inlire';
		$this->method_title   = __( 'in-Lire', 'payment-inlire-gateway2-steps-order' );
		$this->method_description = sprintf( __( 'in-Lire works by adding payment fields on the checkout and then sending the details to in-Lire for verification.', 'payment-inlire-gateway2-steps-order' ) );
	...
	public function get_icon() {
		...
		$icons_str .= sprintf( '&nbsp;<a href="%1$s" class="about_inlire" onclick="javascript:window.open(\'%1$s\',\'InLire\',\'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=1060, height=700\'); return false;">' . esc_attr__( 'What is in-Lire?', 'payment-inlire-gateway2-steps-order' ) . '</a>', esc_url( $this->get_payment_icon_url() ) );
	...
	public function payment_fields() {
		...
		if ( $total >= $this->inlireMaximumAmount  ) {
			$description .= ' ' . sprintf(__( 'Attention. The maximum amout for this payment method is %s', 'payment-inlire-gateway2-steps-order' ), wc_price( $this->inlireMaximumAmount ));
		}
	...
	public function elements_form() {
		...
		<fieldset id="wc-<?php echo esc_attr( $this->id ); ?>-form" class="wc-credit-card-form wc-payment-form" style="background:transparent;">
			...
			<label for="inlire-username"><?php esc_html_e( 'Username', 'payment-inlire-gateway2-steps-order' ); ?> <span class="required"></span></label>
			<div>
				<input type="text" class="input-text " name="inlire-username" id="inlire-username" placeholder="User Name" value="">
			</div>
			<br/>
			<label for="inlire-pwd"><?php esc_html_e( 'Password', 'payment-inlire-gateway2-steps-order' ); ?> <span class="required"></span></label>
			<div>
	...
	public function process_payment( $order_id, $retry = true, $force_save_source = false, $previous_error = false ) {
		try {
			...
			$inlireUsername = sanitize_text_field($_POST['inlire-username']);
			$inlirePassword = sanitize_text_field($_POST['inlire-pwd']);
			
			WC_InLire_Logger::log( 'UserName: ' . $inlireUsername );
			WC_InLire_Logger::log( 'Password: ' . $inlirePassword );
			WC_InLire_Logger::log( 'Url: ' . $this->inlireServiceUrl );
			WC_InLire_Logger::log( 'TK: ' . $this->inlireTokenKey );
			...
			$data_string = json_encode($data,JSON_HEX_QUOT);
			WC_InLire_Logger::log( "DATA: ".$data_string." - SIZE:" . strlen($data_string) );
			...
			if (!is_wp_error($result)) {
				WC_InLire_Logger::log( 'RES: ' . wp_remote_retrieve_body($result) );
				$jdata = json_decode(wp_remote_retrieve_body($result),true);
				if (!empty($jdata)) {
					if ($jdata['isSuccess']) {
						//save the order
