...
abstract class WC_InLire_Payment_Gateway extends WC_Payment_Gateway_CC {
	...
	public function validate_maximum_order_amount( $order ) {
		if (empty($this->inlireMaximumAmount) && !is_numeric($this->inlireMaximumAmount)) {
			throw new WC_InLire_Exception( 'Maximum amount not set', __( 'Set an appropriate maximum amount for the orders', 'payment-inlire-gateway2-steps-order' ) );
		} else {
			if ($this->inlireMaximumAmount<=0) {
				throw new WC_InLire_Exception( 'Maximum amount greater then 0', __( 'Maximum amount must be greater then 0', 'payment-inlire-gateway2-steps-order' ) );
			} else {
				if ( $order->get_total() >= $this->inlireMaximumAmount ) {
					throw new WC_InLire_Exception( 'Did not meet maximum amount', sprintf( __( 'Sorry, the maximum allowed order total is %1$s to use this payment method.', 'payment-inlire-gateway2-steps-order' ), wc_price( $this->inlireMaximumAmount ) ) );
				}
			}
		}
	}
	...
	public function validate_url( $url ) {
		if ( empty($url) ) {
			throw new WC_InLire_Exception( 'Url not set', __( 'Before to use this payment method, the in-Lire url must be set', 'payment-inlire-gateway2-steps-order' ) );
		}
	}
	...
	public function validate_token( $token ) {
		if ( empty($token) ) {
			throw new WC_InLire_Exception( 'Url not set', __( 'Before to use this payment method, the in-Lire token must be set', 'payment-inlire-gateway2-steps-order' ) );
		}
	}
	...
	public function validate_username_and_password( $username, $password ) {
		if ( (trim($username) === '') || (trim($password) === '') ) {
			if (trim($username) === '') {
				throw new WC_InLire_Exception( 'Invalid username', __( 'Please, enter your in-Lire username', 'payment-inlire-gateway2-steps-order' ));
			} else if (trim($password) === '') {
				throw new WC_InLire_Exception( 'Invalid password', __( 'Please, enter your in-Lire password', 'payment-inlire-gateway2-steps-order' ));
			}
		}
	}
